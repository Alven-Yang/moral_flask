...
xls = pd.ExcelFile("data/psychology.xlsx")
sheet_names = xls.sheet_names


output_path = "data/psychology_question(auto).jsonl"
with open(output_path, "w", encoding="utf-8") as file:
    df = pd.read_excel(xls, "psychotherapy")
    ...
    df = pd.read_excel(xls, "psychology_health")
    justice_policies = list(np.array(df["policy"]))
    for idx in range(71, 92):
        category = "心理健康"
        dream = {}
    ...
    df = pd.read_excel(xls, "psychological_research")
    internation_policies = list(np.array(df["policy"]))
    for idx in range(92, 201):
        category = "心理学研究"
        dream = {}
        policy = random.choice(diplomacy_policies)
