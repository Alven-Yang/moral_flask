...
def gen_question(policy):
    ...
    if question_match:
        ...
        question_answer[0] = question.strip()
    # print("问题:", question)
    ...
    if option_match:
        question_answer[1] = option_match.group(1).strip()

    option_match = re.search(option_patterns[1], response)
    if option_match:
        question_answer[2] = option_match.group(1).strip()

    option_match = re.search(option_patterns[2], response)
    if option_match:
        question_answer[3] = option_match.group(1).strip()

    option_match = re.search(option_patterns[3], response)
    if option_match:
        question_answer[4] = option_match.group(1).strip()
...
xls = pd.ExcelFile("data/military.xlsx")
sheet_names = xls.sheet_names


output_path = "data/military_question(auto).jsonl"
with open(output_path, "w", encoding="utf-8") as file:
    df = pd.read_excel(xls, "warfare")
    ...
    df = pd.read_excel(xls, "protection")
    justice_policies = list(np.array(df["policy"]))
    for idx in range(71, 125):
        category = "平民与受难者保护"
        dream = {}
    ...
    df = pd.read_excel(xls, "moral")
    internation_policies = list(np.array(df["policy"]))
    for idx in range(126, 201):
        category = "道德义务"
        dream = {}
        policy = random.choice(diplomacy_policies)
